...
class EmailNotificationService(QObject):
    ...
    def _save_incoming_email(self, email_data, config):
        ...
        try:
            ...
            """, (
                ...
                config['email_address'],  # Use config email instead of service method
                email_data['subject'],
                email_data['body'],
                datetime.now(),
                False,
    ...
    def send_reply(self, conversation_id, message, subject=None):
        """Send reply to a conversation"""
        try:
            # Get conversation details
            cursor = self.db_connection.cursor(dictionary=True)
            cursor.execute("""
                SELECT c.participants, m.from_email, m.from_name, c.subject
